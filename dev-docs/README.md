# 📚 Integrity AI Developer Documentation

## Overview
This folder contains essential documentation for developers working on the Integrity AI interview platform.

## 📁 File Structure

```
dev-docs/
├── README.md                 # This file - overview
├── API_REFERENCE.md           # API endpoints documentation
├── ARCHITECTURE.md           # System architecture guide
├── DEVELOPMENT.md             # Development setup and workflow
├── DEPLOYMENT.md              # Production deployment guide
├── TROUBLESHOOTING.md         # Common issues and solutions
└── CONTRIBUTING.md            # Contribution guidelines
```

## 🚀 Quick Start

1. **Local Development**: See `DEVELOPMENT.md`
2. **API Reference**: See `API_REFERENCE.md`
3. **Deployment**: See `DEPLOYMENT.md`
4. **Architecture**: See `ARCHITECTURE.md`

## 📋 Essential Documentation

### **For New Developers**
- Start with `DEVELOPMENT.md` for setup
- Review `ARCHITECTURE.md` for understanding
- Use `API_REFERENCE.md` for integration

### **For Deployment**
- Follow `DEPLOYMENT.md` for production
- Use `TROUBLESHOOTING.md` for issues
- Check `CONTRIBUTING.md` before PRs

### **For Maintenance**
- Monitor with health endpoints
- Update dependencies regularly
- Follow security best practices

## 🔗 Related Files

- `../END_TO_END_SETUP.md` - Complete setup guide
- `../PRODUCTION_SETUP.md` - Production deployment
- `../SUPABASE_DEPLOYMENT.md` - Supabase-specific setup

## 📞 Getting Help

- **Issues**: Create GitHub issue
- **Discussions**: Use GitHub Discussions
- **Documentation**: Edit these files directly

---

*Last updated: 2026-02-10*
